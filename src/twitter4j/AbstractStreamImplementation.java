                ...
                public void run() {
                    ...
                    if (line != null && line.length() > 0) {
                        try {
                            ...
                            if (JSONObjectType.Type.SENDER == jsonObjectType) {
                                onSender(json);
                            } else if (JSONObjectType.Type.STATUS == jsonObjectType) {
                                onStatus(json);
                            } else if (JSONObjectType.Type.DIRECT_MESSAGE == jsonObjectType) {
                                onDirectMessage(json);
                            } else if (JSONObjectType.Type.DELETE == jsonObjectType) {
                                onDelete(json);
                            } else if (JSONObjectType.Type.LIMIT == jsonObjectType) {
                                onLimit(json);
                            } else if (JSONObjectType.Type.STALL_WARNING == jsonObjectType) {
                                onStallWarning(json);
                            } else if (JSONObjectType.Type.SCRUB_GEO == jsonObjectType) {
                                onScrubGeo(json);
                            } else if (JSONObjectType.Type.FRIENDS == jsonObjectType) {
                                onFriends(json);
                            } else if (JSONObjectType.Type.FAVORITE == jsonObjectType) {
                                onFavorite(json.getJSONObject("source"), json.getJSONObject("target"), json.getJSONObject("target_object"));
                            } else if (JSONObjectType.Type.UNFAVORITE == jsonObjectType) {
                                onUnfavorite(json.getJSONObject("source"), json.getJSONObject("target"), json.getJSONObject("target_object"));
                            } else if (JSONObjectType.Type.RETWEET == jsonObjectType) {
